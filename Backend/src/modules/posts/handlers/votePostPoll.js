...
export function votePostPollInternal(params, body, userId, authorization) {
  return withPostParticipation(params, body, userId, authorization, 'poll', async ({ client, block, option }) => {
    // Remove this user's selection only in this block, including any legacy
    // multiple selections. The replacement and result counts are one transaction.
    await client.query(
