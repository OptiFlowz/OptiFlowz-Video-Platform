...
test('mobile player settings keeps native menus in a modal and restores desktop controls and scrolling', t => {
  dom(t);
  ...
  window.matchMedia = query => query.includes("prefers-reduced-motion") ? { matches: reducedMotion } : media;
  ...
  // Query uses the actual Mux theme tag; a minimal shadow host is sufficient here.
  const themeHost = document.createElement('media-theme');
  playerRoot.append(themeHost);
  const themeRoot = themeHost.attachShadow({ mode: 'open' });
  themeRoot.innerHTML = '<media-controller><media-settings-menu anchor="auto" hidden><media-playback-rate-menu slot="submenu" hidden></media-playback-rate-menu></media-settings-menu><media-settings-menu-button tabindex="0"></media-settings-menu-button></media-controller>';
  const menu = themeRoot.querySelector('media-settings-menu');
  const button = themeRoot.querySelector('media-settings-menu-button');
  const { setupMobilePlayerSettings } = modules()('app/components/playPage/playerCollection/mobilePlayerSettings.ts');
  document.documentElement.style.overflow = 'auto';
  const cleanup = setupMobilePlayerSettings(player, { settings: 'Podešavanja', close: 'Zatvori' });
  const dialog = themeRoot.querySelector('dialog');
  assert.equal(dialog.querySelector(".sheet-close").getAttribute("aria-label"), "Zatvori");
  assert.ok(dialog.querySelector(".sheet-close svg"));
  button.click();
  assert.equal(dialog.open, true);
  assert.equal(menu.parentElement, dialog);
  assert.equal(menu.hidden, false);
  assert.equal(menu.hasAttribute('anchor'), false);
  assert.equal(dialog.getAttribute('aria-label'), 'Podešavanja');
  ...
  button.click();
  dialog.dispatchEvent(new Event('close')); // Stale native event after a rapid reopen.
  assert.equal(dialog.open, true);
  assert.equal(menu.parentElement, dialog);
  dialog.dispatchEvent(new window.KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  assert.equal(dialog.open, false);
  const invoke = new Event('invoke');
  invoke.relatedTarget = button;
  menu.dispatchEvent(invoke); // Keyboard activation from Media Chrome.
  assert.equal(dialog.open, true);
  media.matches = false;
  mediaListeners.forEach(fn => fn());
  assert.equal(dialog.open, false);
  let desktopClicks = 0;
  button.addEventListener('click', () => desktopClicks++);
  button.click();
  assert.equal(desktopClicks, 1);
  assert.equal(dialog.open, false);
  media.matches = true;
  reducedMotion = false;
  button.click();
  dialog.querySelector('.sheet-close').click();
  assert.equal(dialog.open, true, 'Keep the dialog mounted throughout the exit animation');
  ...
  assert.equal(document.documentElement.style.overflow, 'auto');
  button.click();
  dialog.getBoundingClientRect = () => ({ height: 400 });
  const header = dialog.querySelector('.sheet-header');
  const touch = (type, y, time, target = header) => {
  ...
  touch('touchmove', 125, 200);
  assert.equal(dialog.style.getPropertyValue('--sheet-drag-y'), '25px');
  touch('touchend', 125, 210);
  assert.equal(dialog.open, true, 'Short, slow drags snap back');
  assert.equal(dialog.style.getPropertyValue('--sheet-drag-y'), '');
  menu.style.overflowY = 'auto';
  Object.defineProperty(menu, 'scrollHeight', { value: 800 });
  Object.defineProperty(menu, 'clientHeight', { value: 200 });
  menu.scrollTop = 80;
  touch('touchstart', 100, 300, menu);
  touch('touchmove', 240, 500, menu);
  touch('touchend', 240, 510, menu);
  assert.equal(dialog.hasAttribute('data-closing'), false, 'Scrollable lists retain their gesture');
  touch('touchstart', 100, 600);
  touch('touchmove', 250, 800);
  touch('touchcancel', 250, 810);
  assert.equal(dialog.hasAttribute('data-closing'), false, 'Cancelled gestures must not dismiss');
  touch('touchstart', 100, 900);
  touch('touchmove', 250, 1100);
  touch('touchend', 250, 1110);
  assert.equal(dialog.hasAttribute('data-closing'), true, 'A downward swipe dismisses smoothly');
  dialog.dispatchEvent(transitionEnd);
  assert.equal(dialog.open, false);
  button.click();
  cleanup();
  assert.equal(document.documentElement.style.overflow, 'auto');
  assert.equal(themeRoot.querySelector('dialog'), null);
  assert.equal(themeRoot.querySelector('style'), null);
  assert.equal(menu.parentElement.tagName, 'MEDIA-CONTROLLER');
